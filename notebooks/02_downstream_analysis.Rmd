...
cluster_markers <- FindAllMarkers(seurat,
                                  logfc.threshold = 0.5, #To test all possible genes (quite slow), set this to 0
...
-**avg_log2FC** is the log fold change between the cluster (see *cluster* column) and the aggregate of others
...
```{r}
VlnPlot(seurat, features="CD8A", pt.size=0.25)
```
...
```{r}
RidgePlot(seurat, features="CD8A")
```
...
```{r}
top_genes <- cluster_markers %>% 
  filter(gene %in% VariableFeatures(seurat)) %>% #Heatmap function will only plot genes in @scale.data
  group_by(cluster) %>% 
  top_n(3, avg_log2FC) %>%
  pull(gene) %>%
...
An annoying feature of this heatmap function is that it will only 
...
Its basic usage is to just compare two clusters in a seurat object. Let's compare CD8 T (Cluster 3) against CD4 T cells (Cluster 0)

```{r}
dge_tcells <- FindMarkers(seurat, ident.1 = 3, ident.2 = 0,
                          only.pos=F,
...
```{r}
seurat <- AddModuleScore(seurat, features=gene_sets["GO_INNATE_IMMUNE_RESPONSE"],
                         name="InnateImmuneResponse")
```

Now, this module score is actually stored in our metadata table

```{r}
hist(seurat$InnateImmuneResponse1, breaks=50)
```

And we can use standard visualization tools to look at the score in our data
```{r}
FeaturePlot(seurat, features="InnateImmuneResponse1", cols=c('lightgrey', 'red'), order=T)
```

```{r}
VlnPlot(seurat, features="InnateImmuneResponse1", pt.size=0.1)
```

While there is a lot of variability between other clusters, we see that the score is in fact higher in cluster 3 (CD8) than 0 (CD4). We can see it a bit better if we look at just those clusters:

```{r}
VlnPlot(seurat, features="InnateImmuneResponse1", idents=c(0,3))
```
