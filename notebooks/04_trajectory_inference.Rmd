...
Plotting some of the differentially expressed genes
```{r}
...
subset_de_results <- graph_test(cds_subset, neighbor_graph="principal_graph",
                                cores=2)
sig_genes_subset <- row.names(subset_de_results)[subset_de_results$q_value <= 0.05 &
                                                   subset_de_results$morans_I >= 0.1]
...
```{r}
ggplot(gene_module_df, aes(x=dim_1, y=dim_2)) + geom_point(aes(color=module))
```
